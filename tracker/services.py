...
def complete_upload(intent_id, actor):
    ...
    try:
        with transaction.atomic():
            ...
            if intent.completed_at:
                return intent
            tracker, _ = Tracker.objects.get_or_create(order_id=intent.order_id, order_month=intent.order_month, defaults={"owner_id": intent.owner_id})
...
def rename_tracker(tracker_id, order_id, confirmed=False):
    ...
    target = Tracker.objects.filter(order_id=order_id, order_month=tracker.order_month).exclude(pk=tracker.pk).first()
    if target and not confirmed:
        raise ActionError("ID order sudah ada. Konfirmasi untuk menggabungkan foto.")
    UploadIntent.objects.filter(order_id=tracker.order_id, order_month=tracker.order_month, completed_at__isnull=True).update(order_id=order_id)
    if target:
